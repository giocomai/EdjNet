...
# i = 1
# 
# tables <- read_html(x = HtmlFiles[1]) %>% html_nodes("table")
...
#View((read_html(x = HtmlFiles[2]) %>% html_nodes("table") %>% .[6] %>% html_table(fill = TRUE))[[1]])
```
There are 425 films for which effectively no data is available (e.g. [http://lumiere.obs.coe.int/web/film_info/?id=21695]).
...
for (i in seq_along(allFilms)[is.na(allFilms)==FALSE]) {
  if (is.element(el = "Distributor", set = colnames(allFilms[[i]]))==TRUE&is.element(el = "Release date", set = colnames(allFilms[[i]]))==TRUE) {
    allFilmsLong[[i]] <- allFilms[[i]] %>% gather(Year, Admissions, 4:(ncol(allFilms[[i]])-3)) %>% mutate(ID = i, Title = names(filmLinks)[i], Admissions = as.character(Admissions), Link = filmLinks[i]) %>% select(ID, Title, everything())
  } else if (is.element(el = "Distributor", set = colnames(allFilms[[i]]))==TRUE) {
    allFilmsLong[[i]] <- allFilms[[i]] %>% gather(Year, Admissions, 3:(ncol(allFilms[[i]])-3)) %>% mutate(ID = i, Title = names(filmLinks)[i], `Release date` = NA, Admissions = as.character(Admissions), Link = filmLinks[i]) %>% select(ID, Title, everything())
  } else {
    allFilmsLong[[i]] <- allFilms[[i]] %>% gather(Year, Admissions, 2:(ncol(allFilms[[i]])-3)) %>% mutate(ID = i, Title = names(filmLinks)[i]) %>% mutate(Distributor = NA, `Release date` = NA, Admissions = as.character(Admissions), Link = filmLinks[i]) %>% select(ID, Title, Market, Distributor, `Release date`, everything())
  }
}
...
if (file.exists("allFilmsDF.rds")==FALSE) {
  allFilmsDF <- bind_rows(x = allFilmsLong[is.na(allFilms)==FALSE]) %>% mutate(Admissions = as.integer(gsub(pattern = " ", replacement = "", x = Admissions)))
...
```{r}
#View(allFilmsDF %>% filter(Market == "RO") %>% filter(str_detect(Year, "Total")) %>% filter(str_detect(ProducingCountry, "US")==FALSE) %>%  group_by(ID, Title) %>% summarise(Admissions) %>% arrange(desc(Admissions)))
```
